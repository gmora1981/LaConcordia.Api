...
{
    // Replica "REPORTE DE VOUCHER POR PAGAR" (FrmReporteUnidad -> "Voucher X Pagar" ->
    // RptUnidadesDetalleVoucher) del sistema de escritorio: pedidos con empresa asignada que
    // aun no tienen voucher generado, filtrados por unidad y rango de fechas.
    public static class ReporteVoucherPagarPdfGenerator
    {
        public static byte[] GenerarPdf(List<ReporteVoucherPagarDTO> pedidos, string? unidad, string? conductor, DateTime desde, DateTime hasta, string usuarioLogueado)
        {
            var doc = Document.Create(container =>
            {
                ...
                container.Page(page =>
                {
                    ...
                    page.Header().Column(headerCol =>
                    {
                        ...
                        headerCol.Item().PaddingTop(8).AlignCenter().Text("REPORTE DE VOUCHER POR PAGAR").Bold().FontSize(14);
                    ...
                    page.Content().PaddingTop(10).Column(col =>
                    {
                        col.Item().Table(table =>
                        {
                            ...
                            table.Header(h =>
                            {
                                ...
                                h.Cell().Text("empresa").Italic().Underline();
                            ...
                            foreach (var p in pedidos)
                            {
                                ...
                                table.Cell().Text(p.Empresa);
                                table.Cell().AlignRight().Text(p.Precio?.ToString("0.00"));
                            }

                            if (pedidos.Count == 0)
                            {
                                table.Cell().ColumnSpan(5).PaddingTop(10).Text("No hay vouchers pendientes de pago para el rango y la unidad seleccionados.")
